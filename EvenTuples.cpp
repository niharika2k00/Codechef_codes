...
//function to calculate nCr ---> (exceeding time but logic is perfect)
ll nCr(ll n, ll r)
{
    return cal_n(n) / (cal_n(r) * cal_n(n - r));
}

// optimised (REDUCED) form of finding the combination nCr
ll nCrMethod2(ll n, ll r)
{
    ll result = 1, i;
...
int main()
{
    ...
    test
    {
        ...
        while (q--)
        {
            ...
            ODD + ODD + EVEN =  EVEN
             */
            totalTuples = nCrMethod2(Even, 3) + (nCrMethod2(Odd, 2) * Even);
